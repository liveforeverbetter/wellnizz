...
wellnizz supports [x402 v2](https://docs.x402.org/) as an optional
...
`Authorization` header, wellnizz uses normal bearer authentication and
...
```bash
curl -i "https://app.wellnizz.com/providers?modality=genetics"
# HTTP/2 402
...
A verified payer address becomes a deterministic, private wellnizz
...
wellnizz fills them from the verified payer.

## Enabled capabilities

The live deployment is authoritative. Read
`GET https://app.wellnizz.com/.well-known/x402.json` for enabled networks,
...
Override prices with a JSON object keyed by wellnizz endpoint ID or route:
...
credentials or payout-wallet private keys. wellnizz needs only public
recipient addresses; payment signing stays with the paying agent.

## Fulfillment guarantees

wellnizz verifies the payment before executing the route, buffers the
...
For paid `POST` routes, wellnizz derives an idempotency key from the signed
